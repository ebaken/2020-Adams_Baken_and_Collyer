...
sizes for these purposes. A standardized effect size is found as:
...
(Glass 1976; Kelley and Preacher 2012). Here, \(\theta_{obs}\) and
...
distribution obtained from permutation procedures. \hfill\break
...
\tightlist
\item
  Z-score. could be Lambda or Kappa. Show it is Kappa -Comparing the
  strength of physig
\item
  two sample Z-score
\end{itemize}
...
Adams and Collyer 2016, 2019a) based on \(Kappa\).

Variation in the set of \(Z_K\) at each input level of phylogenetic
signal was then calculated as an estimate of precision in \(Z_K\).
However, because \(Z_K\) differs in scale from \(\lambda\), we used a
linear normalization to standardize \(Z_K\) to a uniform distribution
(\(0\rightarrow1\)), and estimated the precision of \(Z_K\) from the
normalized values. \hfill\break
