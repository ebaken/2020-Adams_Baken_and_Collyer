...
\({\hat{\lambda}\) (\(\sigma_{\hat{\lambda}}\)) at each input level of
phylogenetic signal and level of species richness as an estimate of
precision. Random data samples were generated from a normal
distribution, i.e., \(\mathcal{N}(\mu=0,\sigma=\sigma^2\mathbf{C})\). We
verified that the level of \(\sigma^2\) had no effect on
\(\hat{\lambda}\) or \(\sigma^2_{\hat{\lambda}}\), but the results we
present used \(\sigma^2=1\). \hfill\break
...
superifical Bernoulli traits innacuracy precision
