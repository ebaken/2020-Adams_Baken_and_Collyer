...
%DIF DEL PNAS-Adams-01.tex       Sat Jul 18 12:07:48 2020
%DIF ADD PNAS-Adams-01-EKB.tex   Wed Aug 12 10:04:35 2020
...
%DIF 77c77
%DIF < Here we evaluate the efficacy of of Pagel's \(\lambda\) to correctly
%DIF -------
Here we evaluate the efficacy --of-- of Pagel's \(\lambda\) to correctly %DIF > 
%DIF -------
...
By contrast, for Blomberg's \(\kappa\), which is positively \DIFdelbegin \DIFdel{ubounded}\DIFdelend \DIFaddbegin \DIFadd{uNbounded}\DIFaddend ,
...
for adjudicating the \DIFdelbegin \DIFdel{relibalilty }\DIFdelend \DIFaddbegin \DIFadd{--relibalilty-- RELIABILITY }\DIFaddend of these statistics.
Furthermore, \DIFdelbegin \DIFdel{these
}\DIFdelend \DIFaddbegin \DIFadd{--these }\DIFaddend are statistics that appear to have expected values
that vary with tree \DIFaddbegin \DIFadd{size-- the expected values of these statistics
appear to vary with tree }\DIFaddend size (18), making comparisons across studies
...
\begin{figure}
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics{new.fig.1.temp.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{fig.1.png}
\DIFaddendFL \caption{Response of Pagel's \(\lambda\) to increasing strength of
...
\begin{figure}
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics{new.fig.2.temp.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{fig.2.png}
\DIFaddendFL \caption{Response of Blomberg's \(\hat\kappa\) to increasing strength of
Brownian motion. Gray line signifies the 1:1 line where the input value
...
increases more linearly with \DIFdelbegin \DIFdel{incrasing }\DIFdelend \DIFaddbegin \DIFadd{incrEasing }\DIFaddend levels of phylogenetic signal.
...
\begin{figure}
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics{new.fig.3.temp.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{fig.3.png}
\DIFaddendFL \caption{Response of effect sizes \(Z_{\lambda}\) and \(Z_{\kappa}\) to
...
\begin{figure}
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics{new.fig.4.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{fig.4.png}
\DIFaddendFL \caption{(A) Linear measures for relative body size, and regions of the
...
repeated for both \DIFdelbegin \DIFdel{bananced }\DIFdelend \DIFaddbegin \DIFadd{--bananced-- BALANCED }\DIFaddend and pectinate trees, which
...
relative body width (\(\frac{BW}{SVL}\)) measures were obtained \DIFdelbegin \DIFdel{from as species means }\DIFdelend \DIFaddbegin \DIFadd{--from
as species means-- }\DIFaddend from individuals of 305 species, from which species
