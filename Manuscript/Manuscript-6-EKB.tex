...
detected and under what conditions (see also Revell et al. 2008; Revell
2010; e.g., Münkemüller et al. 2012; Pavoine and Ricotta 2012;
Diniz-Filho et al. 2012; Adams 2014a; Molina-Venegas and Rodríguez
2017). One of the most widely used methods for characterizing
phylogenetic signal in macroevolutionary studies is Pagel's \(\lambda\)
(Pagel 1999). The \sout{(\(\lambda\))} parameter (\(\lambda\))
transforms the lengths of the internal branches of the phylogeny to
improve \sout{a} THE fit of data to the phylogeny via maximum likelihood
(Pagel 1999; Freckleton et al. 2002). Pagel's \(\lambda\) ranges from
\(0\to1\), with larger values signifying a greater dependence of
observed trait variation on the phylogeny. Pagel's \(\lambda\) also has
the appeal that it may be included in phylogenetic generalized
least-squares regression (PGLS) to account for the degree of
...
analysis, \(\lambda\) can function as a descriptive statistic \sout{to
describe} CHARACTERIZING the relative strength of phylogenetic signal in
phenotypic traits to determine the extent to which shared evolutionary
history has influenced trait covariation among taxa. \sout{The appeal}
THIS DESCRIPTIVE UTILITY of \(\lambda\) \sout{as a descriptive statistic
for evolutionary biologists} is \sout{a} THE basis for interpreting
``weak'' versus ``strong'' phylogenetic signal\sout{,} (i.e., small
versus large values of \(\lambda\), respectively)\sout{,} in a
comparative sense (e.g., De Meester et al. 2019; Pintanel et al. 2019;
Su et al. 2019). Indeed, statements regarding the strength of
phylogenetic signal based on \(\lambda\) are rather common in the
...
phenotypic trait. Further, because nearly half of the 1,572 \(\lambda\)
values reported were near 0 or 1 (Figure 1) where the biological
interpretation of \(\lambda\) is known, this percentage is even higher.
\hfill\break

{[}insert Figure 1 here{]} \hfill\break

Various other approaches use \(\lambda\) as a parameter that can be
\sout{varied} ADJUSTED for inferences akin to sensitivity analysis. For
instance, some have performed likelihood ratio tests that compare
observed model fits to those obtained when \(\lambda=0\) or
\(\lambda=1\) (Freckleton et al. 2002; Cooper et al. 2010; Bose et al.
2019) or evaluated whether observed \(\lambda\) differs from an expected
\(\lambda\), based on confidence intervals generated for the expected
value (Vandelook et al. 2019). Qualitative comparisons of \(\lambda\)
estimates have also been performed for multiple traits on the same
phylogenetic tree to infer whether the strength of phylogenetic signal
is greater in one trait as compared to another (e.g., Liu et al. 2019;
Bai et al. 2019). \hfill\break
...
observation it was concluded that insufficient data (i.e., \sout{the}
LOW number of species) was the underlying cause of the increased
variation across parameter estimates (Boettiger et al. 2012). Indeed,
such a pattern is common with statistical estimators, as summary
statistics and parameters are often more precise at greater sample sizes
(Cohen 1988). However, this conclusion also implies that the precision
of \(\lambda\) remains constant across its range
...
of the precision with which \(\lambda\) can estimate levels of
...
comparisons of estimates across datasets be interpretable. \sout{For the
case of \(\lambda\), this} HOWEVER, THE CALCULATION AND STATISTICAL
BEHAVIOR OF \(\lambda\) AS AN EFFECT SIZE has not yet been explored.
\hfill\break
...
precisely characterize \sout{observed} KNOWN levels of phylogenetic
signal in phenotypic datasets, and that biological interpretations of
the strength of phylogenetic signal based on \(\lambda\) may be highly
inaccurate. \hfill\break
...
precision of \(Z_\kappa\) were up to an order of magnitude smaller
\sout{for} than for \(Z_\lambda\) (Figure 4C), implying that estimates
of the strength of phylogenetic signal were more reliable and robust
when using \(Z_\kappa\). \hfill\break
...
PHENOTYPIC dataset. \sout{, resulting in a phylogeny and corresponding
phenotypic dataset containing 305 species.} The phylogenetic signal in
each trait was then characterized using \(\kappa\), which was converted
to its effect size (\(Z_\kappa\)) using \texttt{geomorph} 3.2.1 (Adams
and Otárola-Castillo 2013; Adams et al. 2020). Finally, the strength of
phylogenetic signal was compared across traits using \(\hat{Z}_{12}\) as
described above (\textbf{to be incorporated in \texttt{geomorph} upon
manuscript acceptance}). \hfill\break
...
quantify phylogenetic signal (Gittleman and Kot 1990; e.g., Abouheif
...
\(Z_\kappa\) more accurately tracked known levels of phylogenetic
...
such as meta-analysis (Glass 1976; sensu Hedges and Olkin 1985; Arnqvist
...
\(Z_\kappa\) as an effect size\sout{, and}. FURTHERMORE, because
comparison of effect sizes in a test is a comparison of locations of
observed values in their sampling distributions, \sout{if the sampling
distributions also} WHICH WOULD shift EQUALLY because of this tendency,
the \(Z_{12}\) test statistic in equation 4 appears to be robust in
spite of unresolved trees. \hfill\break  
...
\hypertarget{ref-AdamsCollyer2019b}{}
...
\hypertarget{ref-AdamsCollyer2016}{}
...
\hypertarget{ref-AdamsCollyer2018b}{}
...
\hypertarget{ref-Beaulieu_et_al2012}{}
...
\hypertarget{ref-ButlerKing2004}{}
...
\hypertarget{ref-DinizFilho2012}{}
...
\hypertarget{ref-Freckleton_et_al2002}{}
...
\hypertarget{ref-GarlandIves2000}{}
...
\hypertarget{ref-HedgesOlkin1985}{}
...
\hypertarget{ref-Klingenberg2010}{}
...
\hypertarget{ref-MartinsHansen1997}{}
...
\hypertarget{ref-MolinaVenegas2017}{}
...
\hypertarget{ref-Munkemuller_et_al2012}{}
...
\hypertarget{ref-OMeara_et_al2006}{}
...
\hypertarget{ref-RevellHarmon2008}{}
...
\hypertarget{ref-Revell_et_al2008}{}
