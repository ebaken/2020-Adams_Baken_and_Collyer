...
Additionally, because nearly half of the 1,572 \(\lambda\) values
...
signal at the limits of \(\lambda\) are known. \hfill\break
...
strength. Additionally, the same estimated values of \(\lambda\) may be
obtained from datasets containing vastly different input levels of
phylogenetic signal. Thus, \(\lambda\) is not a reliable estimate of the
strength of phylogenetic signal in phenotypic data. We subsequently
derive a standardized effect size for measuring the strength of
phylogenetic signal in phenotypic datasets, and apply the concept to two
common measures of phylogenetic signal: \(\lambda\) and \(Kappa\).
Through simulations across a wide range of conditions, we find that the
...
\(\lambda_{in} = 0.25 \to 0.65\)). Note that these findings were
particularly unsettling when considered in light of our literature
survey. Over one quarter of the \(\lambda\) estimates obtained in
empirical studies (421 of 1,572) were between \(\lambda=0.25\) and
\(\lambda=0.75\) (Figure 1). Of those, 30\% were interpreted directly in
terms of the strength of phylogenetic signal that they represented
(e.g., small, intermediate, strong signal). \hfill\break
...
Information). Additionally, estimates of phylogenetic signal were less
precise, and the spread of \(\lambda_{est}\) was similar to that
observed when \(\lambda\) was estimated for only the dependent variable
(Figure 3). Taken together, these findings reveal that \(\lambda_{est}\)
does not precisely characterize observed levels of phylogenetic signal
in phenotypic datasets, and that biological interpretations of the
strength of phylogenetic signal based on \(\lambda\) may be highly
...
permutation. Here we formalize that suggestion, resulting in an effect
size of:
...
\(Z_\lambda\) (Figure 4A,B). Thus, actual changes in the strength of
phylogenetic signal are reflected more evenly in the corresponding
values of the effect size \(Z_K\). Second, the precision of \(Z_K\) is
considerably more stable as compared with \(Z_\lambda\). This may be
seen by calculating the coefficients of variation for the set of
precision estimates (i.e., \(\sigma^2_{Z_K}\) and
\(\sigma^2_{Z_\lambda}\)) across input levels of phylogenetic signal.
Here coefficients of variation in the precision of \(Z_K\) were an order
of magnitude smaller for than for \(Z_\lambda\) (Figure 4C,D). This
implied that estimates of the strength of phylogenetic signal were more
reliable and robust when using \(Z_K\) as compared with \(Z_\lambda\).
\hfill\break
...
measurement cancel out during their calculation {[}\textbf{S\&R??}{]}.
As a consequence, the effect sizes represent the strength of
...
the original phenotypic variables under investigation.

\hypertarget{section}{%
\subsubsection{}\label{section}}

Conceptually, this is akin to the conversion of data to their standard
normal deviates prior to their use in multivariate analysis, when the
original variables are expressed in different units or scales {[}see
{]}. \#\#\#

\hypertarget{section-1}{%
\paragraph{}\label{section-1}}

mathematical transformation or standardization of the trait dimensions,
so the variables are expressed in similar units n the variables in a
multivariate data set are on different scales,

Everitt, Brian; Hothorn, Torsten J (2011), An Introduction to Applied
Multivariate Analysis with R, Springer,

Also, Legendre\ldots{} Adams and Collyer. \#\#\#\#

Because of this standardization, the strength of phylogenetic signal
across datasets has meaning

something about comparisons having meaning\ldots{}

As such, characterizing the strength of phylogenetic signal with \(Z_K\)
facilitates a formal statistical comparison of values across continuous
traits of different units, scale, or even dimensionality (e.g.,
univariate traits using \(Kappa\), and multivariate traits using
\(K_{mult}\)).
