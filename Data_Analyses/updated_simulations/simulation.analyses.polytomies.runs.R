...
#### Pure-birth with polytomies tree analyses ------------------------------------------
...
### Arguments for PB tree analyses with polytomies 20% but can change %

sim.args <- list(treesize = treesizes[1], tree.type = "pb",
                nsim = nsim, lambdas = lambdas, poly.per = 0.2,
...
sim.sets_32 <- read.csv("Data_Analyses/updated_simulations/results/pb.32.50.sim.0.2.poly.mean.type.csv")
sim.sets_64 <- read.csv("Data_Analyses/updated_simulations/results/pb.64.50.sim.0.2.poly.mean.type.csv")
sim.sets_128 <- read.csv("Data_Analyses/updated_simulations/results/pb.128.50.sim.0.2.poly.mean.type.csv")
sim.sets_256 <- read.csv("Data_Analyses/updated_simulations/results/pb.256.50.sim.0.2.poly.mean.type.csv")
sim.sets_512 <- read.csv("Data_Analyses/updated_simulations/results/pb.512.50.sim.0.2.poly.mean.type.csv")
sim.sets_1024 <- read.csv("Data_Analyses/updated_simulations/results/pb.1024.50.sim.0.2.poly.mean.type.csv")

sim.sets <- list(sim.sets_32, sim.sets_64, sim.sets_128, sim.sets_256, sim.sets_512, sim.sets_1024)
names(sim.sets) <- treesizes

png(filename = "Manuscript/SupportingInfo_Figs/fig.1.poly.png",width = 800, height = 1000, units = "px", pointsize = 18)
par(mfrow = c(3, 2), 
    mar = c(5,5,1,2))
...
### plot kappas
png(filename = "Manuscript/SupportingInfo_Figs/fig.2.poly.png",width = 800, 
...
png(filename = "Manuscript/SupportingInfo_Figs/fig.3.poly.png",width = 800, 
    height = 1000, units = "px", pointsize = 18)
