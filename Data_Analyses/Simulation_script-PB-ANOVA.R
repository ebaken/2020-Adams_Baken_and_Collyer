...
for (j in 1:(length(lambdas_expanded))) {
  ...
  while(TRUE){
    ...
    tree_scaled <- geiger::rescale(tree, model = "lambda", lambda = lambdas_expanded[j])
    }
    
    if(lambdas_expanded[j] == 0) { # without this bit, lambda always estimated as 0.5, and it doesn't make sense
    tree_scaled <- geiger::rescale(tree, model = "lambda", lambda = 0.01) 
    }
    
    X <- rTraitDisc(tree_scaled, model = "ER", states = c(0,1))
    Y_correlated <- lapply(1:length(beta), function(i) {as.numeric(X)*beta[i] + sim.char(tree_scaled, par = 1, nsim = 1)[,,1]})
